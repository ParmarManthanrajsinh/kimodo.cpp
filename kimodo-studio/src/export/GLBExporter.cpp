...
bool GLBExporter::exportAnimation(const Animation& animation,
    ...
    for (int j = 0; j < J; ++j) {
        ...
        for (int f = 0; f < F; ++f) {
            const float* q =
                animation.localRotationsXyzw.data() + (static_cast<size_t>(f) * J + j) * 4;
            writeF32(bin, q[0]);
            writeF32(bin, q[1]);
            writeF32(bin, q[2]);
            writeF32(bin, q[3]);
    ...
    for (int f = 0; f < F; ++f) {
        const float* p =
            animation.rootPositions.data() + static_cast<size_t>(f) * 3;
        writeF32(bin, p[0] * options.rootScale);
        writeF32(bin, p[1] * options.rootScale);
        writeF32(bin, p[2] * options.rootScale);
    }
    pad4();
    ...
    for (int j = 0; j < J; ++j) {
        ...
        const auto& o = animation.offsets[j];
