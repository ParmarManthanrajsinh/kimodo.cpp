...
bool queryStatus(HINTERNET req, long& status) {
    DWORD code = 0;
...
bool queryHeader(HINTERNET req, DWORD id, std::string& out) {
    DWORD len = 0;
    WinHttpQueryHeaders(req, id, WINHTTP_HEADER_NAME_BY_INDEX, WINHTTP_NO_OUTPUT_BUFFER,
                        &len, WINHTTP_NO_HEADER_INDEX);
    if (GetLastError() != ERROR_INSUFFICIENT_BUFFER || len == 0) {
        return false;
    }
    std::wstring w(len / sizeof(wchar_t), 0);
    if (!WinHttpQueryHeaders(req, id, WINHTTP_HEADER_NAME_BY_INDEX, w.data(), &len,
                             WINHTTP_NO_HEADER_INDEX)) {
        return false;
    }
...
long getOnce(const std::wstring& startHost, const std::wstring& startPath,
    ...
    for (int hop = 0; hop < 6; ++hop) {
        WinHttpHandle session(
            WinHttpOpen(L"KimodoStudio/1.0", WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,
        ...
        WinHttpHandle req(WinHttpOpenRequest(conn.h, L"GET", path.c_str(), nullptr,
                                             WINHTTP_NO_REFERER,
        ...
        WinHttpSetOption(req.h, WINHTTP_OPTION_DISABLE_FEATURE, &noRedirect,
                         sizeof(noRedirect));

        const bool sameHost = (host == L"huggingface.co");
        if (!token.empty() && sameHost) {
            const std::wstring auth = L"Authorization: Bearer " + widen(token);
            WinHttpAddRequestHeaders(req.h, auth.c_str(), static_cast<DWORD>(-1),
                                     WINHTTP_ADDREQ_FLAG_ADD);
        }
        if (resumeOffset > 0 && bodyFile) {
            char range[64];
            std::snprintf(range, sizeof(range), "Range: bytes=%llu-",
                          static_cast<unsigned long long>(resumeOffset));
            WinHttpAddRequestHeaders(req.h, widen(range).c_str(), static_cast<DWORD>(-1),
                                     WINHTTP_ADDREQ_FLAG_ADD);
        }
        ...
        if (status == 301 || status == 302 || status == 303 || status == 307 ||
            status == 308) {
            std::string location;
            if (!queryHeader(req.h, WINHTTP_QUERY_LOCATION, location)) {
                error = "redirect without Location";
                return 0;
            }
            if (location.rfind("https://", 0) == 0) {
                std::wstring newHost, newPath;
                if (!crackUrl(location, newHost, newPath)) {
        ...
        if (status == 200 && resumeOffset > 0 && bodyFile) {
            // Server ignored Range: restart from scratch.
            return -1;
        }
        std::string lengthStr;
        if (queryHeader(req.h, WINHTTP_QUERY_CONTENT_LENGTH, lengthStr)) {
            try {
                contentTotal = std::stoull(lengthStr);
                if (status == 206) {
                    contentTotal += resumeOffset;
                }
            } catch (...) {
            }
        }
        ...
        do {
            ...
            while (chunk > 0) {
                ...
                if (bodyFile) {
                    if (std::fwrite(buf.data(), 1, got, bodyFile) != got) {
                ...
                chunk -= got;
                if (progress && bodyFile && !progress(done, contentTotal)) {
                    error = "cancelled";
                    return -2;
                }
            }
...
FHuggingFaceClient::HttpResult FHuggingFaceClient::apiGet(const std::string& path,
    ...
    uint64_t total = 0;
    const long status =
        getOnce(L"huggingface.co", widen(path), token, out.body, nullptr, 0, total, none, error);
    if (status > 0) {
        out.status = status;
        if (status != 200) {
            error = "HTTP " + std::to_string(status);
...
bool FHuggingFaceClient::download(const std::string& url, const std::string& tmpPath,
    ...
    for (int attempt = 0; attempt <= retries; ++attempt) {
        ...
        {
            FILE* probe = nullptr;
            if (fopen_s(&probe, tmpPath.c_str(), "rb") == 0 && probe) {
        ...
        if (fopen_s(&f, tmpPath.c_str(), append ? "ab" : "wb") != 0 || !f) {
            error = "cannot open temp file";
            return false;
        }
        std::string body; // unused for downloads
        uint64_t total = 0;
        long status = getOnce(host, path, token, body, f, offset, total, progress, error);
        fclose(f);
        if (status == -1) {
